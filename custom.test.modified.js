...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      console.log(aliceKeypair.pubkey);
      // regular L1 deposit: Alice deposits into tornado pool
      ...
      // withdraws a part of his funds from L2 to recepient 0xDeaD00000000000000000000000000000000BEEf
      const aliceWithdrawAmount = utils.parseEther('0.08')
      // withdraw the amount from a random address.
      ...
      await transaction({
        ...
        recipient: recipient,
        isL1Withdrawal: false, //withdrawal from L2 required
      })

    const recipientBalance = await token.balanceOf(recipient)
    //print the recipient/withdrawal balance
    ...
    //print the omniBridge balance which should be equal to 0 since the balance has been withdrawn.
    ...
    //it should print the remaining balance in the pool after the withdrawal.
    console.log('tornadoPoolBalance = ', (tornadoPoolBalance / 1000000000000000000))
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    //withdraw the reamaining amount from the pool
    const aliceWithdrawAmount = utils.parseEther((aliceDepositAmount.sub(bobSendAmount)/1000000000000000000).toString())
    const recipient = '0xb04E80CA9C994251Fa54b9B7FBd3590B1F2597F8'
    ...
    //Recipient/Alice's balance should be 0 and omniBridge balance should be equal to remaining amount withdrawn by Alice since transaction takes place in L1
     const recipientBalance = await token.balanceOf(recipient)
     console.log('recipientBalance = ', (recipientBalance / 1000000000000000000))
